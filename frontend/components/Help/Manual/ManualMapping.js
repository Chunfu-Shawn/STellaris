    ...
    return (
        <div className="modal-body-stw" style={contentStyle}>
            ...
            <Typography style={{marginTop:50,fontSize:16}}>
                ...
                <p>
                    To prioritize ST sections matching user-specified organs or tissues and to help users find the properly
                    matched ST section, we introduced a screening method called section blast by assessing the similarity
                    between user-uploaded scRNA-seq and the ST datasets in our local database, as inspired by the multimodal
                    intersection analysis (MIA) methods approach described in (
                    <a target={"_blank"} href={"https://doi.org/10.1038/s41587-019-0392-8"} rel={"noreferrer"}>
                        https://doi.org/10.1038/s41587-019-0392-8
                    </a>)
                </p>
                <p>
                    In this case, we used a hypergeometric test to measure the overlap of marker genes (log-transformed
                    fold change &gt;1 and q value &lt;0.05) between all scRNA-seq cell types and all clusters (Leiden clustering;
                    resolution: 1) in the ST data, with the number of intersections of all detected genes in the two
                    modalities as the background for determining the p value. The Benjamini-ochberg method was used for
                    multiple testing correction. To better interpret these p value results, the following calculations
                    were applied: If the p value was less than or equal to 0.5, we performed -np.log10(p) transformation;
                    otherwise, we performed np.log10(1-p). Finally, the average of the highest value in each scRNA-seq
                    cell type was deemed the metric we called MIA score to measure the similarity between the scRNA-seq
                    and the examined ST data.
                </p>
                ...
                <p>
                    ...
                    caused by the
                    incompatibility between the two modalities, we introduced a filtering method, coembedding filtering,
                    by filtering cells that are not well mixed with ST spots in shared UMAP space. Specifically, for all
                    ST spots, the average Euclidean distance between each spot and the default 50 nearest neighbours was
                    calculated, and the mean of average distances of all spots was determined as a cutoff value that
                    measures the proximity of cells to ST spots in shared UMAP space. Cells that were not within the
                    scope of any ST spots were filtered.
                </p>
                ...
                <p>
                    ...
                    </a>. Briefly, this method trains a multivariate RF model on ST data in the shared latent space and then
                    applies the model to the coembedding data of the scRNA-seq and ST data, thereby enabling the prediction
                    of spatial coordinates of single cells by assigning them to the reciprocal nearest ST spots where the
                    similarity is measured by the RF distance metric.
                </p>
                <p>
                    We made some adaptations for thorough evaluation, higher reliability and faster speed, which are as follows:
                </p>
                <ul>
                    <li>We used 50 PCs when training the RF model;</li>
                ...
                <p>
                    ...
                    any two cell types using Euclidean distance as described in (40). We then performed k-means clustering
                    (k=3) on the spatial distance of all pairs of cell types, thereby dividing cell type pairs into three
                    distance groups: &quot;near,&quot; &quot;medium&quot; and &quot;far.&quot;
                </p>
                <h2>6. Advanced parameters</h2>
                ...
                <ol>
                    ...
                    <li>
                        <div>
                            ...
                            <p>
                                b. Dot figures for total or each cell type in a group (dot_plot.pdf, dot_plot_[group]|[cell type].pdf)
                                similar to web graphs, where means of the average expression level of two interacting
